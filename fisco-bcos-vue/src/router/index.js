    ...
    children: [
      {
        path: 'roles',
        name: 'Roles',
        component: () => import('@/views/clientPermission/roles'),
        meta: { title: '积分设置', icon: 'el-icon-s-tools' }
      }
    ]
  },
  {
...
export const bankRoutes = [
  {
    ...
    children: [
      ...
      },*/
      {
        path: 'complex-table',
        name: 'ComplexTable',
        component: () => import('@/views/table/complex-table -bank'),
        meta: { title: '积分处理' }
      }]
  }
]
  ...
  if (to.path === '/login') {
    ...
    /*if (store.getters.token) {
      ...
      if (hasRoles) {
        ...
        try {
          //const { roles } = await store.dispatch('user/_getInfo',store.getters.status)
          const addRoutes = store.dispatch(
            'permission/getAsyncRoutes',
            //roles
          )
          router.addRoutes(addRoutes)
