  ...
  <div class="rolesControl">
    <el-card>
      ...
      >
        ...
        <el-table-column prop="accept" label="状态" width="90">
          <template slot-scope="scope">
            <el-tag :type="scope.row.accept | tagClass">{{
              scope.row.accept | statusText
              }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            <el-button
                    ...
                    :disabled="scope.row.accept === 0 ? false : true"
            >Accept</el-button>
          </template>
        </el-table-column>
...
<script>
  ...
  import { bankChuli} from '@/api/roles'
    ...
    methods: {
      _getAllRolse() {
        ...
        getAllBanks()
                .then(res => {
                  ...
                  //store.commit('SET_LENGTH',res.response.length)
      ...
      toSuccess(row) {
        ...
        let params= {
          ...
          score:row.score,
          idbankBusinessId:row.idbankBusinessId
        }
        bankChuli(params)
                .then(res => {
                  //localStorage.setItem('length',res.response.length)
