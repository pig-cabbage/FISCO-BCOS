<template>
  <div class="goods">
    <div class="nav">
      <div class="w">
        <a href="javascript:;" :class="{active:sortType===1}" @click="reset()">综合排序</a>
        <a href="javascript:;" @click="sortByPrice(1)" :class="{active:sortType===2}">价格从低到高</a>
        <a href="javascript:;" @click="sortByPrice(-1)" :class="{active:sortType===3}">价格从高到低</a>
        <div class="price-interval">
          <input type="number" class="input" placeholder="价格" v-model="min">
          <span style="margin: 0 5px"> - </span>
          <input type="number" placeholder="价格" v-model="max">
          <y-button text="确定" classStyle="main-btn" @btnClick="reset" style="margin-left: 10px;"></y-button>
          <input type="text" class="input" placeholder="模糊搜索" v-model="goodsName">
          <y-button text="搜索" classStyle="main-btn" @btnClick="search" style="margin-left: 10px;"></y-button>
        </div>
      </div>
    </div>

    <div v-loading="loading" element-loading-text="加载中..." style="min-height: 35vw;">
      <div class="img-item" v-if="!noResult">
        <!--商品-->
        <div class="goods-box w">
          <mall-goods v-for="(item,i) in goods" :key="i" :msg="item"></mall-goods>
        </div>


        <el-pagination
          v-if="!noResult&&!error"
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
          :current-page="currentPage"
          :page-sizes="[8, 20, 40, 80]"
          :page-size="pageSize"
          layout="total, sizes, prev, pager, next, jumper"
          :total="total">
        </el-pagination>
      </div>
      <!--<div class="no-info" v-if="noResult">
        <div class="no-data">
          <img src="@/assets/images/no-search.png">
          <br> 抱歉！暂时还没有商品
        </div>
        <section class="section">
          <y-shelf :title="recommendPanel.name">
            <div slot="content" class="recommend">
              <mall-goods :msg="item" v-for="(item,i) in recommendPanel.panelContents" :key="i"></mall-goods>
            </div>
          </y-shelf>
        </section>
      </div>
      <div class="no-info" v-if="error">
        <div class="no-data">
          <img src="@/assets/images/error.png">
          <br> 抱歉！出错了...
        </div>
        <section class="section">
          <y-shelf :title="recommendPanel.name">
            <div slot="content" class="recommend">
              <mall-goods :msg="item" v-for="(item,i) in recommendPanel.panelContents" :key="i"></mall-goods>
            </div>
          </y-shelf>
        </section>
      </div>-->
    </div>
  </div>
</template>
<script>
  import { getAllGoods , productDet} from '@/api/goods.js'
  //import { recommend } from '/api/index.js'
  import mallGoods from '@/components/mallGoods'
  import YButton from '@/components/YButton'
  //import YShelf from '/components/shelf'
  export default {
    data () {
      return {
        goods: [],
        goodsBig: [],
        noResult: false,
        error: false,
        min: '',
        max: '',
        goodsName: '',
        loading: true,
        timer: null,
        sortType: 1,
        windowHeight: null,
        windowWidth: null,
        recommendPanel: [],
        sort: '',
        currentPage: 1,
        total: 0,
        pageSize: 20
      }
    },
    methods: {
      handleSizeChange (val) {
        this.pageSize = val
        this._getPageGoods()
        this.loading = true
      },
      handleCurrentChange (val) {
        this.currentPage = val
        this._getPageGoods()
        this.loading = true
      },
      _getPageGoods() {
          let start = (this.currentPage - 1) * this.pageSize
          let end = start + this.pageSize
          this.goods = this.goodsBig.slice(start, end)
      },
      _getAllGoods () {
        //let cid = this.$route.query.cid
        if (this.min !== '') {
          this.min = Math.floor(this.min)
        }else {
          this.min = 0
        }
        if (this.max !== '') {
          this.max = Math.floor(this.max)
        }else {
          this.max = 1000000
        }
        let params = {
          begin:this.min,
          end:this.max
        }
        getAllGoods(params).then(res => {
          if (res) {
            this.total = res.response.length
            this.goods = res.response
            console.log(res.response[0].goodsUrl)
            this.goodsBig=this.goods
            this._getPageGoods()
          }
        })
      },
      // 默认排序
      reset () {
        this.sortType = 1
        this.sort = ''
        this.currentPage = 1
        this.loading = true
        this._getAllGoods()
      },
      search(){
        let name = {
          name: this.goodsName
        }
        productDet(name).then(res => {
          if (res) {
            this.total = res.response.length
            this.goods = res.response
            console.log(res.response[0].goodsUrl,res.response.length)
            this.goodsBig=this.goods
          }
        })
      },
      // 价格排序
      sortByPrice (v) {
        v === 1 ? this.sortType = 2 : this.sortType = 3
        this.sort = v
        this.currentPage = 1
        this.loading = true
        this._getAllGoods()
      }
    },
    watch: {
      /*$route (to, from) {
        if (to.fullPath.indexOf('/goods?cid=') >= 0) {
          this.cId = to.query.cid
          this._getAllGoods()
        }
      }*/
    },
    created () {
        this._getAllGoods()
    },
    mounted () {
      this.windowHeight = window.innerHeight
      this.windowWidth = window.innerWidth
      this._getAllGoods()
      /*recommend().then(res => {
        let data = res.result
        this.recommendPanel = data[0]
      })*/
    },
    components: {
      mallGoods,
      YButton,
      //YShelf
    }
  }
</script>
<style lang="scss" rel="stylesheet/scss" scoped>
  @import "../../style/mixin";
  @import "../../style/theme";

  .nav {
    height: 60px;
    line-height: 60px;
    > div {
      display: flex;
      align-items: center;
      a {
        padding: 0 15px;
        height: 100%;
        @extend %block-center;
        font-size: 12px;
        color: #999;
        &.active {
          color: #5683EA;
        }
        &:hover {
          color: #5683EA;
        }
      }
      input {
        @include wh(80px, 30px);
        border: 1px solid #ccc;
      }
      input + input {
        margin-left: 10px;
      }
    }
    .price-interval {
      padding: 0 15px;
      @extend %block-center;
      input[type=number] {
        border: 1px solid #ccc;
        text-align: center;
        background: none;
        border-radius: 5px;
      }
    }
  }

  .goods-box {
    > div {
      float: left;
      border: 1px solid #efefef;
    }
  }

  .no-info {
    padding: 100px 0;
    text-align: center;
    font-size: 30px;
    display: flex;
    flex-direction: column;
    .no-data{
      align-self: center;
    }
  }

  .img-item{
    display: flex;
    flex-direction: column;
  }

  .el-pagination{
    align-self: flex-end;
    margin: 3vw 10vw 2vw;
  }

  .section {
    padding-top: 8vw;
    margin-bottom: -5vw;
    width: 1218px;
    align-self: center;
  }

  .recommend {
    display: flex;
    > div {
      flex: 1;
      width: 25%;
    }
  }

  .searchDiv {
    margin-bottom: 20px;
    .el-button {
      padding: 11px 20px;
    }
  }

  .width1 {
    width: 280px;
    margin-right: 10px;
  }

</style>
